...
def load_training_parameters():
    file_path = '../../datasets/dataset_training/triplet_v50_splits'
    ...
    config = SimpleNamespace(
        ...
        TRAIN_DATASET_PATH = f'{file_path}/train',
        EVAL_DATASET_PATH = f'{file_path}/eval',
        TEST_DATASET_PATH = f'{file_path}/test',
        ...
        DO_EVAL = True,
        SEED = 412,
        LR = 2e-05, # default value 1e-5 for BGE, 2e-5 for pubmed_bert # LR is multipled by math.sqrt(SCALING_FACTOR)
        EPOCHS = 3,
        TRAIN_BATCH_SIZE = 256, #32 #256    # train batch size is multipled by SCALING_FACTOR
        EVAL_BATCH_SIZE = 32,
        SCALING_FACTOR = 1,
...
def load_dataset_pandas(config) -> pd.DataFrame:
    ...
    eval_data = eval_data[['anchor', 'positives', 'negatives']].drop_duplicates()
    test_data = test_data[['anchor', 'positives', 'negatives']].drop_duplicates()
...
def build_huggingface_dataset(config):
    ...
    train_data = load_from_disk(config.TRAIN_DATASET_PATH)
    eval_data = load_from_disk(config.EVAL_DATASET_PATH)
    test_data = load_from_disk(config.TEST_DATASET_PATH)

    train_data = train_data.remove_columns('triplet_length')
    eval_data = eval_data.remove_columns('triplet_length')
    test_data = test_data.remove_columns('triplet_length')
...
def main(config, date_time):
    ...
    # ************************* new code *************************
    N = len(train_dataset)
    world_size = int(os.environ.get("WORLD_SIZE","1")) # depending on what value we set here : torchrun --nproc_per_node=2, currently 2
