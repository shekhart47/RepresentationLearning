...
# torchrun --nproc_per_node=2 optimized_training_script_v4.py
...
def build_huggingface_dataset(config):
    ...
    def add_triplet_length(example):

        anchor_len = len(tokenizer(example['anchor'])['input_ids'])
        positive_len = len(tokenizer(example['positives'])['input_ids'])
        negative_len = len(tokenizer(example['negatives'])['input_ids'])
        example['triplet_length'] = max(anchor_len, positive_len, negative_len)
        return example

    train_dataset = train_dataset.map(add_triplet_length, batched = False)
    train_dataset = train_dataset.sort('triplet_length')
...
def get_SentenceTransformerTrainingArguments(config):
    ...
    args = SentenceTransformerTrainingArguments(
        ...
        batch_sampler= config.BATCH_SAMPLER,  # GPT Solution: Use length-grouped batching across triplets
...
#    optimizer = transformers.AdamW(model.parameters(),lr=config.LR, weight_decay = config.WEIGHT_DECAY)
    ...
    #model_path = '../../../shekhar_tanwar/ICD-ICD-Triplet/model/e5-large-v2-20250331143312-finetuned-icd-v30/'
    #collator = TripletCollator(model_path = model_path, pad_to_multiple_of=8)
    ...
    trainer = SentenceTransformerTrainer(
                ...
                #data_collator=collator,    # GPT Solution: NEW: dynamic triplet collator
